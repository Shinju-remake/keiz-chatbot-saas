(function() {
    ...
    if (window.self !== window.top && (window.location.pathname.includes('/demo') || window.location.pathname.includes('/dashboard'))) {
        return; // Prevent duplicate widget inside the Console iframes
    }
    ...
    async function startRecording() {
        try {
            ...
            mediaRecorder.onstop = async () => {
                ...
                statusBanner.innerText = "● ANALYZING...";
                try {
                    const res = await fetch(TRANSCRIBE_URL, { method: 'POST', headers: { "X-API-Key": API_KEY }, body: formData });
